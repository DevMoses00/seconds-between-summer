...
func _ready() -> void:
	...
	clues_found.connect(orb_popup_end)
...
func _handle_new_clue(from_index: int, to_index: int, at_position: Vector2) -> void:
	...
	# this is to mark when to track the amount of clues selected
	tracker_on = true
	mini_tracker += 1
...
func _handle_clue_clicked(clue_data: SourceClue) -> void:
	...
	var sfx_select = ["Keyword1","Keyword2","Keyword3","Keyword4"].pick_random()
	SoundManager.play_sfx(sfx_select,0,-10)
	mini_tracker -= 1
...
func orb_popup_end():
	root.on_all_keywords_collected()
